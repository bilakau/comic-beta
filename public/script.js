...
async function checkAuthStatus() {
    ...
    updateAuthUI();
    if(user) {
        // Otomatis pindahkan history lokal ke cloud saat login
        syncLocalToCloud();
    }
}
...
function updateAuthUI() {
    ...
    if(currentUser) {
        ...
        userName.innerText = profile.full_name || "Pembaca";
        desc.innerHTML = `<i class="fa fa-circle text-green-500 mr-1"></i> Cloud Sync Aktif. Riwayat aman & tersinkronisasi.`;
        avatar.innerHTML = `<img src="${profile.avatar_url}" class="w-full h-full object-cover">`;
...
// --- SYNC LOGIC (Pindahkan Lokal ke Akun) ---

async function syncLocalToCloud() {
    if(!currentUser) return;
    let localHistory = JSON.parse(localStorage.getItem('fmc_history') || '[]');
    
    if(localHistory.length === 0) return;

    // Masukkan semua data lokal ke Supabase
    const toSync = localHistory.map(item => ({
    ...
    const { error } = await supabase.from('history').upsert(toSync, { onConflict: 'user_id, slug' });
    
    if(!error) {
        console.log("History lokal berhasil disinkronkan ke Cloud!");
    }
}
...
async function saveHistory(slug, title, image, chSlug, chTitle) {
    ...
    // 1. Simpan ke LocalStorage (Selalu simpan sebagai backup)
...
async function showHistory() {
    ...
    if(currentUser && supabase) {
        ...
        if(!error) {
            displayData = data.map(d => ({
                ...
                title: d.title,
                image: d.image,
                lastChapterSlug: d.last_chapter_slug,
    ...
    renderGrid({ data: displayData }, currentUser ? "Riwayat Cloud (Sinkron)" : "Riwayat Lokal");
}
...
document.addEventListener('DOMContentLoaded', () => {
    loadGenres();
    checkAuthStatus(); // Cek login saat web dibuka
    handleInitialLoad();
});
